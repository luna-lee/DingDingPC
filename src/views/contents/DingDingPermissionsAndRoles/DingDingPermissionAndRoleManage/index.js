...
import permission from '../components/DingDingPermissionManage/DingDingPermissionManage'
import role from '../components/DingDingRoleManage/DingDingRoleManage'
		...
		getRoleTreeData() {
			...
			this.$ajax.get(this.path + '/json/mis_getData', {
				...
				if(data.success) {
					...
					initRoot[tables.role.colum.name] = "钉钉角色管理";
				...
				if(data.success) {
					...
					tree.forEach(function(value) {
						...
						if(!value[tables.permission.colum.parentid]) {
							...
							value[tables.permission.colum.name] = "钉钉权限管理";
						}
					})
		...
		getPermissionByRole() {
			...
			this.$ajax.get(this.path + '/json/mis_getData', {
				...
				if(data.success) {
					...
					initRoot[tables.permission.colum.name] = "当前钉钉角色所拥有的权限";
					initRoot.expand = true;
					...
					tree.forEach(function(value) {
						...
						if(!value[tables.permission.colum.parentid]) {
							rootExist = true;
							$this.root = value[tables.permission.colum.pk];
							value[tables.permission.colum.name] = "当前钉钉角色所拥有的权限";
						}
					})

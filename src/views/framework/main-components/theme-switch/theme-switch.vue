    ...
    <div style="display:inline-block;padding:0 6px;">
        <Dropdown trigger="click" @on-click="setTheme">
            <a href="javascript:void(0)">
                ...
                <Icon size="20" style="margin-left: -10px;" type="md-arrow-dropdown"></Icon>
            </a>
            <DropdownMenu slot="list">
                <DropdownItem v-for="(item, index) in themeList" :key="index" :name="item.name">
                    <Row type="flex" justify="center" align="middle">
                        <span style="margin-right:10px;"><Icon :size="20" :type="item.name.substr(0, 1) !== 'b' ? 'md-star-outline' : 'md-star'" :color="item.menu"/></span>
...
export default {
    ...
    methods: {
        setTheme (themeFile) {
            ...
            if (config.env.indexOf('dev') > -1) {
                stylePath = './src/views/framework/main-components/theme-switch/theme/';
            } else {
                stylePath = 'dist/';
            }
            if (mainTheme !== 'b') {
    ...
    created () {
        let path = '';
        if (config.env.indexOf('dev') > -1) {
            path = './src/views/framework/main-components/theme-switch/theme/';
        } else {
            path = 'dist/';
        }
